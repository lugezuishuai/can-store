...
fetch('products.json').then(function(response){
    return response.json();     //返回的相应是json格式字符串
}).then(function(json){
    let products = json;
    initialize(products);
}).catch(function(err){
...
function initialize(products){
    ...
    //finalGroup是category和search之后将要被展示的产品对象的数组
    let finalGroup;
    ...
    function selectCategory(e){
        ...
        if(category.value === lastCategory && searchTerm.value.trim() === lastSearch){        //string.trim(): 用于删除字符串的头尾空格
            return;
        }else{
            ...
            if(category.value === 'All'){
                ...
                // the values in the <option> elements are uppercase, wheres the categories
                ...
                //Run selectProducts() after the filterring has been done
                selectProducts();
            }
        }
    }

    //selectProducts() Takes the group of products selected by sleectCategory(), and further 
    //filters them by the tiered search term (if ones has been entered)
    ...
    function showProduct(objectURL, product){
        ...
        // as 1.40, not 1.4.
        para.textContent = '$' + product.price.toFixed(2);      //toFixed(2): 保留两位小数
